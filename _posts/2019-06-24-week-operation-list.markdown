...
분류 |목표     | 진행상황     | 문제점    |    해결방안   
----- | -------------- | -------------- | -------------------- | ------------  
1 | openmpi device 병렬 실행 | 50% | NFS 설정과 openssh설정 완료 하고 접속까지는 성공했는데 실행방법에 문제가 있음. | 1.mpirun의 실행명령에 대한 공부<br> 2.slot 추가?? 에 대한 내용 찾아보기
1 | SSA코드 병렬 실행 | 미진행 | 1.아직 device 병렬 실행이 진행되지 않았음<br>2. device 별로 작업단위를 어떻게 할 것인지에 대해 질의가 필요함. | 교수님께 질의
...
분류 |목표     | 진행상황     | 문제점    |    해결방안   
----- | -------------- | -------------- | -------------------- | ------------  
1 |  openmpi 공부 | 50% | 기본적인 mpi 프로그래밍만 진행. <br>프로세스단위 동시실행은 가능하나, device 별로는 못함. | 교수님께 질의 및 관련자료 더 찾아보기
1+ |  openmpi & NFS 설치 | 100% | device별로 실행하는 방법을 모르겠음 | 교수님께 질의 및 관련자료 더 찾아보기
1+ | SSA코드 병렬 실행 | 미진행 | 1.mpi에 대한 개념이 부족<br> 2.ssa코드에 대한 이해가 부족해서 병렬실행 코드 작성 시도를 못함. | 그래도 시도 해보기!<br> 하지만 device별 동시실행을 우선순위로 둘 것.
1+ | JetsonNano devicequery 결과에 대한 nvidia에 질의하기 | 80 % | 답변이 안옴.... | .......
1+ | Udacity cuda 강의 | 0.8% | 500개의 강의중 40개만 들었음.. | 부지런히 들을 것
2 | openmpi device 병렬 실행 |  | 어떻게 해야 할 지 감이 안선다.. |
2 | SSA코드 병렬 실행 |  | 어떻게 해야 할 지 감이 안선다..|
